...
```
T1d_bbpe_10k/               # This experiment — Byte-level BPE tokenizer, 10K vocab
...
At 10K vocab, `RFI` becomes a single token and `preconstruction` splits cleanly at the morpheme boundary — improvements over the 5K baseline. The `Ġ` prefix artifact is cosmetic and fully reversible via `tokenizer.decode()`. Generation coherence is unchanged: all tokenizer variants produce fragment recitation. The limiting factor is training data structure, not tokenization.
